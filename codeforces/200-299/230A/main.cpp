...
struct comp
{
    bool operator()(dragon const &d1, dragon const &d2)
    {
        // cout << "str " << d1.strength << " " << d2.strength << endl;
...
// can run multiple test cases in one shot while debugging locally, codeforces will still only run the code one time
...
#endif
    {
        ...
        ll x, y;
        cin >> s >> n;
        priority_queue<dragon, vector<dragon>, comp> q;
        for (auto i = 0; i < n; i++)
        {
            cin >> x >> y;
            q.push(dragon(x, y));
        ...
        while (!q.empty())
        {
            ...
            // cout << temp.strength << " " << temp.bonus << endl;
            q.pop();
        }
        if (won)
        {
